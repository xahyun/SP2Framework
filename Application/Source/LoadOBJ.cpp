...
)
{
	...
	while (!fileStream.eof()) {
		...
		if (strncmp("v ", buf, 2) == 0) { //process vertex position
			...
			TexCoord vertex;
			sscanf_s((buf + 3), "%f%f", &vertex.u, &vertex.v);
...
bool LoadOBJMTL(const char* file_path, const char* mtl_path, std::vector<Position>& out_vertices, std::vector<TexCoord>& out_uvs, std::vector<Vector3>& out_normals, std::vector<Material>& out_materials)
{
	...
	while (!fileStream.eof()) {
		...
		if (strncmp("v ", buf, 2) == 0) { //process vertex position
			Position vertex;
			sscanf_s((buf + 2), "%f%f%f", &vertex.x, &vertex.y, &vertex.z); temp_vertices.push_back(vertex);
		}
		else if (strncmp("vt ", buf, 3) == 0) { //process texcoord
			TexCoord texCoord;
			sscanf_s((buf + 2), "%f%f", &texCoord.u, &texCoord.v); temp_uvs.push_back(texCoord);
		}
		else if (strncmp("vn ", buf, 3) == 0) { //process normal
			Vector3 normal;
			sscanf_s((buf + 2), "%f%f%f", &normal.x, &normal.y, &normal.z); temp_normals.push_back(normal);
		}
