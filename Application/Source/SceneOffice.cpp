...
SceneOffice::SceneOffice()
{
	...
	PlaySound(TEXT("C:\\Users\\Daphne\\Music\\SP2 music\\Office"), NULL, SND_SYNC);
...
void SceneOffice::Init()
{
	...
	//office 
	{
		...
		meshList[GEO_OFFICEFLOOR] = MeshBuilder::GenerateOfficefloor("quad", Color(1, 1, 0), 1.f);
...
void SceneOffice::RenderOfficeSpace()
{
...
void SceneOffice::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
	if (!mesh || mesh->textureID <= 0) //Proper error check
		return;
	glDisable(GL_DEPTH_TEST); //uncomment for RenderTextOnScreen
...
void SceneOffice::RenderImageOnScreen(Mesh* mesh, Color color, float sizex, float sizey, float x, float y)
{
	if (!mesh || mesh->textureID <= 0) //Proper error check
		return;
	glDisable(GL_DEPTH_TEST); //uncomment for RenderTextOnScreen
