...
/*!
\brief
Distance between another vector

\param point
	point which you want to find distance between
\return
	Resulting vector
*/
/******************************************************************************/
float Vector3::Distance(const Vector3& point)
{
	return sqrt(abs(x - point.x) * abs(x - point.x) + abs(z - point.z) * abs(z - point.z));
}
...
\param point
	point which you want to find angle between
\return
	Note!! it returns the basic acute (0-90 degrees) angle
*/
/******************************************************************************/
float Vector3::AngleDifference(const Vector3& point)
{
	float angle = Math::RadianToDegree(acos((this->Dot(point) / (this->Length() * point.Length()))));
	return angle;
}
